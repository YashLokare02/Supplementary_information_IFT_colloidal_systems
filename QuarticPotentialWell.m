...
for i= 2:Nt
    ...
    % From eq. (2) and eq. (3)
    x(i) = x(i-1) - 4*alpha*Dt*(x(i-1) - qt(i-1))^3/gamma;  % Gets appended with the position of the trap center at each iteration
...
for i= 2:Nt
    ...
    F(N_p, i) = -4*alpha*(x_disp(N_p, i)).^3;  % The particle is trapped within a harmonic potential near the focal point of the optical trap
end
